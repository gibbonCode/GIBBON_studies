...
savePath=fullfile(defaultFolder,'2022_Maria_Gomez_Cerezo_NPerevoshchikova_PET_scaffold','data','temp');
loadPathMatProp=fullfile(defaultFolder,'2022_Maria_Gomez_Cerezo_NPerevoshchikova_PET_scaffold','data','mat_prop','PET');
...
%Loading the experimental elastic properties
matname = fullfile(loadPathMatProp,'PET.mat');
load(matname);
disp_exp_original=Mat_disp_av;
load_exp_original=Mat_force_av;

modelOrderCase=1;%{0,1,2} for 1st Ogden order (neo-Hookean);2nd Ogden order;neo-Hookean
switch modelOrderCase
    case {0,2} %crop the data to displacement ~ 0.25 mm
        disp_exp=disp_exp_original(1:end-10);
        load_exp=load_exp_original(1:end-10);
    case {1} 
        disp_exp=disp_exp_original(1:end-3);
        load_exp=load_exp_original(1:end-3);
end

cFigure; 
title('Elastic region','fontSize',fontSize); 
hold on;
xlabel('Average displacement, mm','FontSize',fontSize);
ylabel('Average load, N','FontSize',fontSize);
plot(disp_exp_original,load_exp_original,'Color',blue,'LineWidth',4);
plot(disp_exp,load_exp,'k.-','LineWidth',4);
...
switch modelOrderCase
    case 0 %1st Ogden order (neo-Hookean)
        c1_ini=596;
        k_ini=(c1_ini)*k_factor;
        P=[c1_ini];
    case 1 %2nd Ogden order
        c1_ini=5955;
        m1_ini=-2;
        c2_ini=-5197;
        m2_ini=2;
...
switch modelOrderCase
    ...
    case 2
        ...
        febio_spec.Material.material{1}.ATTR.id=1;
        febio_spec.Material.material{1}.E=Young_modulus;
        febio_spec.Material.material{1}.v=v_ratio;
end
...
if runFlag==1 %i.e. a succesful run
    ...
    hl(3)=plot(disp_exp_original,load_exp_original,'Color',blue,'LineWidth',4);
    hl(2)=plot(disp_exp,load_exp,'k-','lineWidth',4);
    hl(1)=plot(disp_sim,load_sim,'r.-','lineWidth',4,'markerSize',markerSize*2);
    patch([xt1 fliplr(xt2)], [f1 fliplr(f2)],0.5.*ones(1,3),'FaceAlpha',0.2,'EdgeColor','None')

    legend(hl,{'Simulation','Experiment crop','Experiment original'},'Location','northwest');
...
switch modelOrderCase
    case 0
        mat_struct.par_names={'c1','k'}; %Parameter names
        mat_struct.par_values={c1_ini k_ini}; %Parameter values
        ...
        objectiveStruct.Pb_struct.xxlim=[[P(1)/1000]' [P(1)*1000]']; %Parameter bounds
    case 1    
        ...
        objectiveStruct.parNormFactors=abs(P); %This will normalize the parameters to ones(size(P))
        objectiveStruct.Pb_struct.xx_c=P; %Parameter constraining centre
        objectiveStruct.Pb_struct.xxlim=[[P(1)/100 -30 P(3)*100   0]'...
                                         [P(1)*100  30  P(3)/100  30]']; %Parameter bounds
    case 2
        mat_struct.par_names={'E'}; %Parameter names
        mat_struct.par_values={Young_modulus}; %Parameter values
...
objectiveStruct.modelOrderCase=modelOrderCase;
...
functionTolerance=0.001; %Tolerance on objective function value
parameterTolerance=1e-25;%Tolerance on parameter variation
...
Hn(1)=plot(OPT_stats_out.disp_sim,OPT_stats_out.load_sim,'r.-','lineWidth',4,'markerSize',markerSize*2);
Hn(3)=plot(disp_exp_original,load_exp_original,'Color',blue,'LineWidth',4);
Hn(2)=plot(disp_exp,load_exp,'k-','lineWidth',4);
patch([xt1 fliplr(xt2)], [f1 fliplr(f2)],0.5.*ones(1,3),'FaceAlpha',0.2,'EdgeColor','None')
    

legend(Hn,{'Simulation','Experiment crop', 'Experiment original'},'Location','northwest');
view(2); axis tight;  grid off;
set(gca,'FontSize',fontSize);
drawnow;
...
cFigure;hold on;
gpatch(Fb,V_DEF(:,:,qt),CV,'k',1,2); %Add graphics object to animate
gpatch(Fb,V,0.5*ones(1,3),'none',0.25); %A static graphics object
...
switch objectiveStruct.modelOrderCase
    case 0
        mat_struct.par_values={P(1) P(1)*objectiveStruct.k_factor};
    case 1
        mat_struct.par_values={P(1) P(2) P(3) P(4) (P(1)+P(3))*objectiveStruct.k_factor};
    case 2
        mat_struct.par_values={P(1)};
end
